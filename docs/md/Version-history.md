...
- The Find toolbar/window has a **Regex** toggle to the left of **Match Case**: when on, the find box's text is matched as a regular expression (e.g. `Kernel.*ZZ`) instead of plain text. Supports literals, `.`, character classes (`[abc]`, `[^abc]`, `[a-z]`, `\d \D \w \W \s \S`), the quantifiers `* + ?` (optionally non-greedy), grouping, alternation (`|`), and the `^` / `$` anchors; a regex match never spans a page break
...
- `CmdFindToggleRegex` : "Find: Toggle Regex" — Find toolbar/window button, to the left of Match Case; the find box's text is matched as a regular expression instead of plain text
